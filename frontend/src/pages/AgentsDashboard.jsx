...
function ScoreRing({ score, size = 64, color = "#7c3aed", label }) {
  const r = (size - 8) / 2;
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width={size} height={size} className="transform -rotate-90">
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="currentColor" strokeWidth="4" className="text-[#333]" />
      ...
      {label && <span className="text-[10px] text-[#737373] mt-1">{label}</span>}
    </div>
  );
}
...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={copy} className="p-1 rounded hover:bg-[#1f1f1f] transition-colors">
      {copied ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-[#555]" />}
    </button>
  );
}
...
function EditorSection({ lang }) {
  ...
  if (fetching) return <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-[#7c3aed]" /></div>;
    ...
    <div data-testid="editor-agent-section">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-[#f0f0f0]">{t.editor}</h2>
        </div>
        <button data-testid="run-editor-btn" onClick={runAgent} disabled={loading}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-[#7c3aed] text-white hover:bg-[#6d28d9] disabled:opacity-50 transition-all">
      ...
      {!report ? (
        <p className="text-sm text-[#555] text-center py-6">{t.noData}</p>
      ) : (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
              <p className="text-xl font-bold text-[#7c3aed]">{report.total_articles}</p>
              <p className="text-[10px] text-[#737373] uppercase tracking-wide">{t.articles}</p>
            </div>
            <div className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
              <p className="text-xl font-bold text-blue-500">{report.hero_picks?.length || 0}</p>
              <p className="text-[10px] text-[#737373] uppercase tracking-wide">{t.heroStories}</p>
            </div>
            <div className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
              <p className="text-xl font-bold text-red-500">{report.duplicate_groups?.length || 0}</p>
              <p className="text-[10px] text-[#737373] uppercase tracking-wide">{t.duplicates}</p>
            </div>
          </div>

          {report.editorial_en && (
            <div className="bg-[#7c3aed]/5 rounded-lg p-4 border border-[#7c3aed]/20">
              <h3 className="text-xs font-bold text-[#a78bfa] uppercase tracking-wide mb-2 flex items-center gap-1.5">
          ...
          {report.hero_articles?.length > 0 && (
            <div>
              <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-2">{t.heroStories}</h3>
              <div className="space-y-1.5">
                {report.hero_articles.map((a, i) => (
                  <div key={i} className="flex items-start gap-2 bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                    <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#7c3aed] text-white text-[10px] font-bold flex items-center justify-center">{i + 1}</span>
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-[#e5e5e5] line-clamp-2">{a.title}</p>
                      <p className="text-[10px] text-[#555]">{a.source}</p>
                    </div>
                  </div>
                ))}
              </div>
...
function TopicCard({ topic, lang, onViewReport }) {
  ...
  return (
    <div data-testid={`topic-${topic.id}`} className="bg-[#0d0d0d] rounded-xl p-4 border border-[#1f1f1f] shadow-sm">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`w-2 h-2 rounded-full ${topic.active ? "bg-green-500" : "bg-[#333]"}`} />
      </div>
      <div className="flex gap-2">
        <button data-testid={`run-topic-${topic.id}`} onClick={runInvestigation} disabled={loading}
          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg bg-[#1a1a1a] text-[#a3a3a3] hover:bg-[#222] disabled:opacity-50 transition-all">
          {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3" />}
          {loading ? t.running : t.runInvestigator}
        </button>
        <button data-testid={`view-report-${topic.id}`} onClick={() => onViewReport(topic.id)}
          className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg bg-[#7c3aed] text-white hover:bg-[#6d28d9] transition-all">
...
function ReportView({ topicId, lang, onBack }) {
  ...
  if (loading) return <div className="flex justify-center py-12"><Loader2 className="w-6 h-6 animate-spin text-[#7c3aed]" /></div>;
  if (!data || !data.report) return <p className="text-sm text-[#555] text-center py-8">{t.noReport}</p>;
  ...
  return (
    <div data-testid="investigation-report-view">
      <button onClick={onBack} className="text-xs text-[#7c3aed] font-medium mb-3 flex items-center gap-1 hover:text-indigo-600">
        &larr; Back to topics
      </button>
      <h2 className="text-lg font-bold text-[#f0f0f0] mb-1">
        {topic.name_en}
      </h2>
      <p className="text-[10px] text-[#555] mb-4">{events.length} {t.events} / {t.lastRun}: {new Date(report.created_at).toLocaleString()}</p>

      <div className="bg-[#7c3aed]/5 rounded-xl p-4 border border-[#7c3aed]/20 mb-4">
        <h3 className="text-xs font-bold text-[#a78bfa] uppercase tracking-wide mb-2 flex items-center gap-1.5">
      ...
      <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-3 flex items-center gap-1.5">
        <Clock className="w-3.5 h-3.5" /> {t.timeline}
      </h3>
      <div className="relative pl-4 border-l-2 border-[#7c3aed]/30 space-y-3">
        {events.map((e, i) => (
          <div key={e.id || i} className="relative">
            <div className="absolute -left-[21px] top-1 w-3 h-3 rounded-full bg-[#7c3aed] border-2 border-white border-[#080808]" />
            <div className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f] shadow-sm">
              <div className="flex items-start gap-2">
                {e.image && <img src={e.image} alt="" className="w-12 h-12 rounded-md object-cover flex-shrink-0" />}
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-[#e5e5e5] line-clamp-2">{e.title}</p>
                  <p className="text-[10px] text-[#737373] mt-0.5 line-clamp-2">{e.summary}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[9px] text-[#7c3aed] font-medium">{e.source}</span>
                    {e.published_at && <span className="text-[9px] text-[#555]">{new Date(e.published_at).toLocaleDateString()}</span>}
                  </div>
                </div>
              </div>
            </div>
    ...
    <div data-testid="investigator-agent-section">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-[#f0f0f0]">{t.investigator}</h2>
        </div>
      </div>
      <div className="space-y-3">
        {topics.map(topic => (
...
function SeoSection({ lang }) {
  ...
  if (fetching) return <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-[#22c55e]" /></div>;
    ...
    <div data-testid="seo-agent-section">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-[#f0f0f0]">{t.seo}</h2>
        </div>
        <button data-testid="run-seo-btn" onClick={runAgent} disabled={loading}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-[#22c55e] text-white hover:bg-[#16a34a] disabled:opacity-50 transition-all">
          {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3" />}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 grid grid-cols-2 gap-2">
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-[#22c55e]">{report.trending_keywords?.length || 0}</p>
                <p className="text-[10px] text-[#737373]">{t.trendingKeywords}</p>
              </div>
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-[#f59e0b]">{report.content_gaps?.length || 0}</p>
                <p className="text-[10px] text-[#737373]">{t.contentGaps}</p>
              </div>
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-blue-500">{report.tweets?.length || 0}</p>
                <p className="text-[10px] text-[#737373]">{t.tweets}</p>
              </div>
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-purple-500">{report.total_articles_analyzed || 0}</p>
                <p className="text-[10px] text-[#737373]">{t.articles}</p>
              </div>
            </div>
          </div>
            ...
            {tabs.map(tb => (
              <button key={tb.id} onClick={() => setTab(tb.id)}
                className={`flex-1 text-[11px] font-semibold py-1.5 rounded-md transition-all ${
                  tab === tb.id ? "bg-[#1a1a1a] text-[#f0f0f0] shadow-sm" : "text-[#737373] hover:text-[#a3a3a3]"
                }`}>{tb.label}</button>
            ))}
          </div>
          ...
          {tab === "overview" && (
            <div className="space-y-3">
              ...
              {report.trending_keywords?.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-2 flex items-center gap-1.5">
                    <Hash className="w-3.5 h-3.5 text-[#22c55e]" /> {t.trendingKeywords}
                  </h3>
                  <div className="flex flex-wrap gap-1.5">
                    {report.trending_keywords.map((kw, i) => (
                      <span key={i} className="px-2 py-1 text-[11px] font-medium rounded-full bg-[#22c55e]/5 text-emerald-700 text-[#22c55e] border border-[#22c55e]/20">
                        {kw}
                      </span>
                    ))}
                  </div>
              ...
              {report.content_gaps?.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-2 flex items-center gap-1.5">
                    <AlertCircle className="w-3.5 h-3.5 text-[#f59e0b]" /> {t.contentGaps}
                  </h3>
                  <div className="space-y-1.5">
                    {report.content_gaps.map((gap, i) => (
                      <div key={i} className="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2.5 border border-amber-100 dark:border-amber-800/30">
                        ...
                        <p className="text-xs text-[#a3a3a3]">{gap}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {report.meta_suggestions?.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-2">SEO Optimization</h3>
                  <div className="space-y-2">
                    {report.meta_suggestions.map((m, i) => (
                      <div key={i} className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
                        <p className="text-[10px] text-[#555] line-through">{m.original_title}</p>
                        <p className="text-xs font-semibold text-[#22c55e] mt-1">{m.optimized_title}</p>
                        <p className="text-[10px] text-[#737373] mt-1">{m.meta_description}</p>
                        <span className="inline-block mt-1 px-1.5 py-0.5 text-[9px] font-medium bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 text-[#22c55e] rounded">{m.focus_keyword}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {tab === "social" && (
            <div className="space-y-3">
                    ...
                    {report.tweets.map((tweet, i) => (
                      <div key={i} className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-xs text-[#a3a3a3] leading-relaxed flex-1">{tweet}</p>
                          <CopyBtn text={tweet} />
                        </div>
                        <p className="text-[10px] text-[#555] mt-1">{tweet.length}/280 chars</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {report.instagram_captions.map((caption, i) => (
                      <div key={i} className="bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-xs text-[#a3a3a3] leading-relaxed flex-1">{caption}</p>
              ...
              {report.hashtag_sets?.length > 0 && (
                <div>
                  ...
                  {report.hashtag_sets.map((set, i) => (
                    <div key={i} className="mb-2 bg-[#0d0d0d] rounded-lg p-3 border border-[#1f1f1f]">
              ...
              {report.best_posting_times?.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-2 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-[#06b6d4]" /> {t.postTimes}
                  </h3>
                  <div className="space-y-1.5">
                    {report.best_posting_times.map((time, i) => (
                      <div key={i} className="flex items-center gap-2 bg-[#06b6d4]/5 rounded-lg p-2.5 border border-[#06b6d4]/20">
                        <Clock className="w-3.5 h-3.5 text-[#06b6d4] flex-shrink-0" />
                        <p className="text-xs text-[#a3a3a3]">{time}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {tab === "strategy" && report.strategy_report && (
            <div className="bg-[#22c55e]/5 rounded-lg p-4 border border-[#22c55e]/20">
              <h3 className="text-xs font-bold text-[#22c55e] uppercase tracking-wide mb-2 flex items-center gap-1.5">
          ...
          {tab === "infra" && (
            <div className="space-y-3">
              {/* SEO Health Score */}
              {seoStats && (
                <div className="bg-[#0d0d0d] rounded-lg p-4 border border-[#1f1f1f]">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xs font-bold text-[#22c55e] uppercase tracking-wide">SEO Health Score</h3>
                    <div className="relative flex items-center justify-center">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <div className="text-center p-2 rounded-lg bg-[#111] bg-[#111]">
                      <p className="text-lg font-bold text-[#22c55e]">{seoStats.recent_24h || 0}</p>
                      <p className="text-[9px] text-[#737373] uppercase">New Today</p>
                    </div>
                    <div className="text-center p-2 rounded-lg bg-[#111] bg-[#111]">
                      <p className="text-lg font-bold text-blue-500">{seoStats.recent_with_seo || 0}</p>
                      <p className="text-[9px] text-[#737373] uppercase">With SEO</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {seoStats && (
                <div className="bg-[#0d0d0d] rounded-lg p-4 border border-[#1f1f1f]">
                  <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-3">Coverage Breakdown</h3>
                  {[
                    ...
                    <div key={i} className="mb-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[11px] font-medium text-[#737373]">{item.label}</span>
                  ...
                  <div className="flex justify-between text-[10px] text-[#555] mt-2 pt-2 border-t border-[#1f1f1f]">
              ...
              <div className="bg-[#0d0d0d] rounded-lg p-4 border border-[#1f1f1f]">
                <h3 className="text-xs font-bold text-[#22c55e] uppercase tracking-wide mb-3">Active SEO Infrastructure</h3>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between py-2 border-b border-[#1f1f1f] last:border-0">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-[#a3a3a3]">{item.label}</p>
                          <p className="text-[10px] text-[#555]">{item.desc}</p>
                        </div>
                      </div>
                      {item.url && (
                        <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[#22c55e] font-medium hover:underline">
                          View
                        </a>
                      )}
                    </div>
              ...
              {/* How it works */}
              <div className="bg-[#22c55e]/5 rounded-lg p-4 border border-[#22c55e]/20">
                <h3 className="text-xs font-bold text-[#22c55e] uppercase tracking-wide mb-2">How Auto-SEO Works</h3>
    ...
    <div data-testid="tech-agent-section">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-[#f0f0f0]">{t.tech}</h2>
        </div>
      ...
      {!report ? (
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 grid grid-cols-2 gap-2">
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-cyan-500">{lh.avg_response_ms || 0}<span className="text-[10px] font-normal text-[#555]">ms</span></p>
                <p className="text-[10px] text-[#737373]">{t.avgResponse}</p>
              </div>
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-[#f59e0b]">{lh.p95_response_ms || 0}<span className="text-[10px] font-normal text-[#555]">ms</span></p>
                <p className="text-[10px] text-[#737373]">{t.p95}</p>
              </div>
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-blue-500">{lh.total_requests || 0}</p>
                <p className="text-[10px] text-[#737373]">{t.requests}</p>
              </div>
              <div className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                <p className="text-lg font-bold text-red-500">{lh.error_count || 0}</p>
                <p className="text-[10px] text-[#737373]">{t.errors}</p>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {report.top_endpoints.map((ep, i) => {
                  ...
                  const barColor = ep.avg_ms > 500 ? "bg-red-500" : ep.avg_ms > 200 ? "bg-amber-500" : "bg-[#22c55e]";
                  return (
                    <div key={i} className="bg-[#0d0d0d] rounded-lg p-2.5 border border-[#1f1f1f]">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-[11px] font-mono text-[#a3a3a3] truncate flex-1">{ep.endpoint}</p>
                        <span className="text-[11px] font-bold text-[#737373] ml-2">{ep.avg_ms}ms</span>
                      </div>
                      <div className="h-1.5 bg-[#1a1a1a] rounded-full overflow-hidden">
                        <div className={`h-full ${barColor} rounded-full transition-all duration-500`} style={{ width: `${barWidth}%` }} />
                      </div>
                      <div className="flex gap-3 mt-1">
                        <span className="text-[9px] text-[#555]">{ep.requests} reqs</span>
                        <span className="text-[9px] text-[#555]">max {ep.max_ms}ms</span>
          ...
          {report.anomalies?.length > 0 && (
            <div>
              <h3 className="text-xs font-bold text-[#737373] uppercase tracking-wide mb-2 flex items-center gap-1.5">
                <AlertCircle className="w-3.5 h-3.5 text-[#f59e0b]" /> {t.anomalies}
              </h3>
              <div className="space-y-1.5">
  ...
  return (
    <div data-testid="agents-dashboard" className="max-w-lg mx-auto px-4 pt-2 pb-24">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <h1 className="text-lg font-bold text-[#f0f0f0]">{t.title}</h1>
      </div>
            ...
            <button key={tab.id} data-testid={`agent-tab-${tab.id}`}
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg whitespace-nowrap transition-all ${
                isActive
                  ...
                  : `bg-[#0d0d0d] text-[#737373] hover:bg-[#1a1a1a]`
              }`}
      ...
      <div className="bg-[#111] bg-[#0d0d0d]/50 rounded-xl p-4">
