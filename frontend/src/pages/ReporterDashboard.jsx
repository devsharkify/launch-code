  ...
  return (
    <div data-testid="reporter-dashboard-page" className={`min-h-screen ${darkMode ? "bg-[#080808]" : "bg-[#111]"}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              {reporter.photo ? (
                ...
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${"bg-[#1a1a1a]"}`}>
                  <User size={24} className={darkMode ? "text-[#555]" : "text-[#737373]"} />
                </div>
              )}
              <div>
        ...
        {reporter.status === "approved" && (
          <div className="flex gap-3 mb-6">
            <Button
              ...
              className="bg-[#7c3aed] hover:bg-[#6d28d9] flex-1"
            >
            ...
            <Button
              ...
              onClick={fetchIdCard}
              variant="outline"
              className={darkMode ? "border-[#262626] text-[#e5e5e5]" : ""}
            >
        ...
        <div className={`rounded-lg border ${darkMode ? "bg-[#0d0d0d] border-[#1f1f1f]" : "bg-[#0d0d0d] border-[#1f1f1f]"}`}>
          ...
          {news.length === 0 ? (
            <div className="p-8 text-center">
              <FileText size={40} className={`mx-auto mb-3 ${darkMode ? "text-[#737373]" : "text-[#555]"}`} />
              <p className={darkMode ? "text-[#555]" : "text-[#737373]"}>
                No news submitted yet
              </p>
            </div>
          ) : (
            <div className="divide-y divide-[#1f1f1f] dark:divide-[#1f1f1f]">
              {news.map((item) => (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-xs ${darkMode ? "text-[#737373]" : "text-[#555]"}`}>
                          &bull; {getTimeAgo(item.created_at)}
                        </span>
                      </div>
                      <h3 className={`font-medium line-clamp-1 ${"text-[#f0f0f0]"}`}>
                        {item.title}
                      </h3>
                      <p className={`text-sm line-clamp-2 mt-1 ${darkMode ? "text-[#555]" : "text-[#737373]"}`}>
                        {item.summary}
                      </p>
                    </div>
      ...
      <Dialog open={showSubmitModal} onOpenChange={setShowSubmitModal}>
        <DialogContent className={`max-w-2xl max-h-[90vh] overflow-y-auto ${darkMode ? "bg-[#0d0d0d] border-[#1f1f1f]" : ""}`}>
          <DialogHeader>
          ...
          <Tabs value={newsForm.news_type} onValueChange={(v) => setNewsForm(prev => ({ ...prev, news_type: v }))}>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="space-y-2">
                <Label className={darkMode ? "text-[#e5e5e5]" : ""}>Title *</Label>
                <Input
                  data-testid="news-title-input"
                  value={newsForm.title}
                  onChange={(e) => setNewsForm(prev => ({ ...prev, title: e.target.value }))}
                  placeholder="News headline"
                  className={darkMode ? "bg-[#1a1a1a] border-[#262626] text-white" : ""}
                />
              </div>

              {/* Summary */}
              <div className="space-y-2">
                <Label className={darkMode ? "text-[#e5e5e5]" : ""}>Summary *</Label>
                <Textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className={darkMode ? "text-[#e5e5e5]" : ""}>Category</Label>
                  <Select
                    value={newsForm.category}
                    onValueChange={(v) => setNewsForm(prev => ({ ...prev, category: v }))}
                  >
                    <SelectTrigger className={darkMode ? "bg-[#1a1a1a] border-[#262626] text-white" : ""}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  <Label className={darkMode ? "text-[#e5e5e5]" : ""}>Location</Label>
                  <Input
              ...
              {/* Image Upload */}
              <div className="space-y-2">
                <Label className={darkMode ? "text-[#e5e5e5]" : ""}>
                  News Image
                </Label>
                <FileUpload
                  type="image"
              ...
              <TabsContent value="video_url" className="mt-0">
                <div className="space-y-2">
                  <Label className={darkMode ? "text-[#e5e5e5]" : ""}>YouTube Video URL *</Label>
                  <Input
              ...
              <TabsContent value="reporter_video" className="mt-0">
                <div className="space-y-2">
                  ...
                  />
                  <p className={`text-xs ${darkMode ? "text-[#555]" : "text-[#737373]"}`}>
        ...
        <DialogContent className="max-w-sm">
          ...
          {idCardData && (
            <div
              ...
              className="bg-gradient-to-br from-[#7c3aed] to-[#4c1d95] rounded-xl p-6 text-white"
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-lg font-bold">LaunchCode</h3>
                  <p className="text-xs text-[#c4b5fd]">Citizen Reporter</p>
                </div>
                <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center">
                  <img src="/tvr-logo.png" alt="LaunchCode" className="w-8 h-8 object-contain" />
                </div>
              </div>
              ...
              <div className="flex gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#c4b5fd]">{idCardData.designation}</p>
                  <p className="text-2xl font-mono font-bold mt-1">{idCardData.reporter_id}</p>
                </div>
              </div>

              <div className="space-y-1 text-sm">
                <p><span className="text-[#c4b5fd]">Location:</span> {idCardData.location || "N/A"}</p>
                <p><span className="text-[#c4b5fd]">Phone:</span> {idCardData.phone}</p>
                <p><span className="text-[#c4b5fd]">Valid Until:</span> {idCardData.valid_until}</p>
              </div>

              <div className="mt-4 pt-4 border-t border-white/20 text-center">
                <p className="text-xs text-[#c4b5fd]">This is a digital ID for LaunchCode citizen reporters</p>
              </div>
            </div>
          )}
