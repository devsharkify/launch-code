...
const EpaperPage = () => {
  ...
  const shareEpaper = async () => {
    const t = `LaunchCode - ${selectedSlot === "morning" ? "Morning" : "Evening"} - ${fmtDate(selectedDate)}`;
  ...
  return (
    <div className="flex flex-col" style={{ background: "#b0b0b0", height: "calc(100vh - 60px)" }} data-testid="epaper-page">
      ...
      <div className="flex items-center justify-center gap-1.5 px-2 py-1 flex-shrink-0 flex-wrap" style={{ background: "#222", borderBottom: "1px solid #444" }}>
        {/* Page nav */}
        <button onClick={() => goToPage(Math.max(0, currentPage - 1))} disabled={currentPage === 0} className="p-0.5 text-[#555] disabled:opacity-30 hover:text-white" data-testid="epaper-prev-page"><ChevronLeft size={14} /></button>
        <span className="text-[11px] text-[#a3a3a3] font-mono">{currentPage + 1}/{pages.length}</span>
        <button onClick={() => goToPage(Math.min(pages.length - 1, currentPage + 1))} disabled={currentPage >= pages.length - 1} className="p-0.5 text-[#555] disabled:opacity-30 hover:text-white" data-testid="epaper-next-page"><ChevronRight size={14} /></button>
        ...
        <div className="relative">
          <button data-testid="epaper-date-picker" onClick={() => setShowDatePicker(!showDatePicker)}
            className="flex items-center gap-1 px-1.5 py-0.5 text-[11px] text-[#a3a3a3] rounded hover:bg-[#1a1a1a] border border-[#262626]">
            <Calendar size={10} />{fmtDateFull(selectedDate)}
          </button>
          {showDatePicker && (
            <div className="absolute right-0 top-full mt-1 z-50 max-h-56 overflow-y-auto w-56 shadow-xl rounded bg-[#0d0d0d] border border-[#1f1f1f]">
              {editions.map(e => {
                ...
                return (
                  <button key={`${e.date}_${e.slot}`} onClick={() => { selectEdition(e.date, e.slot); setShowDatePicker(false); }}
                    className="w-full text-left px-3 py-1.5 text-[11px] flex justify-between items-center hover:bg-[#111] border-b border-[#1a1a1a]"
                    ...
                    <span className="text-[#555] text-[10px]">{e.article_count}</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="w-px h-4 bg-[#262626]" />
        <button data-testid="epaper-share" onClick={shareEpaper} className="p-1 text-[#555] hover:text-white"><Share2 size={13} /></button>
      ...
      <div className="flex-1 overflow-auto" ref={viewerRef} style={{ background: "#a0a0a0" }}>
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full text-[#555] text-sm">No edition available</div>
        ) : (
      ...
      <div className="flex-shrink-0 border-t" style={{ background: "#1a1a1a", borderColor: "#c41e1e" }}>
        <div className="flex items-center gap-1.5 px-3 py-1.5 overflow-x-auto" data-testid="epaper-archive">
          <span className="text-[10px] text-[#555] font-semibold uppercase tracking-wide whitespace-nowrap flex-shrink-0">Editions:</span>
