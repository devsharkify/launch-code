...
export default function ApiKeysManager({ darkMode }) {
  ...
  const bg = darkMode ? "bg-[#111] border-[#1f1f1f]" : "bg-[#0d0d0d] border-[#a3a3a3]";
  const txt = darkMode ? "text-[#e5e5e5]" : "text-[#1a1a1a]";
  const sub = darkMode ? "text-[#a3a3a3]" : "text-[#737373]";

  const baseUrl = window.location.origin;
  const exampleCode = `// Fetch latest news from LaunchCode API
    ...
    <div data-testid="api-keys-manager" className="space-y-4">
      ...
      <div className={`rounded-xl border p-5 ${bg}`}>
        <h3 className={`text-sm font-bold mb-3 flex items-center gap-2 ${txt}`}>
          <Key className="w-4 h-4 text-[#7c3aed]" /> Create API Key
        </h3>
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            ...
            <input
              ...
              className={`w-full mt-1 px-3 py-2 rounded-lg border text-sm bg-[#1a1a1a] border-[#262626] text-white`}
            />
          </div>
          <div className="w-32">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#7c3aed] text-white rounded-lg text-sm font-semibold hover:bg-[#7c3aed] disabled:opacity-50 flex items-center gap-1"
          >
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => copyToClipboard(exampleCode, "code")}
                className={`text-[10px] flex items-center gap-1 px-2 py-1 rounded ${sub} hover:bg-[#e5e5e5] dark:hover:bg-[#1a1a1a]`}>
                {copied === "code" ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />} Copy
              </button>
            </div>
            <pre className={`p-3 rounded-lg text-[11px] font-mono leading-relaxed overflow-x-auto bg-[#0d0d0d] text-[#d4d4d4]`}>
              {exampleCode}
            </pre>
          </div>
