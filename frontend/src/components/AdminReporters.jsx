  ...
  return (
    <div data-testid="admin-reporters-section">
                    ...
                    <TableCell colSpan={5} className="text-center py-8">
                      <div data-testid="reporters-error-state" className="flex flex-col items-center gap-3">
                        ...
                        <Button
                          ...
                          className="text-[#7c3aed] border-orange-300 hover:bg-orange-50"
                        >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          {reporter.photo ? (
                            ...
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-[#262626]`}>
                              <User size={18} className="text-[#737373]" />
                            </div>
                          )}
                          <div>
                            <p className={`font-medium text-white`}>{reporter.name}</p>
                            <p className="text-xs text-[#737373]">{reporter.reporter_id}</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <p className={`text-sm text-[#d4d4d4]`}>{reporter.phone}</p>
                        <p className="text-xs text-[#737373]">{reporter.location || "N/A"}</p>
                      </TableCell>
                      <TableCell>{getStatusBadge(reporter.status)}</TableCell>
                      <TableCell className="text-sm text-[#737373]">{getTimeAgo(reporter.created_at)}</TableCell>
                      <TableCell>
                    ...
                    <TableRow key={news.id} data-testid={`reporter-news-row-${news.id}`}>
                      <TableCell>
                        <div className="max-w-xs">
                          <p className={`font-medium line-clamp-1 text-white`}>{news.title}</p>
                          <p className="text-xs text-[#737373] line-clamp-1">{news.summary}</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <p className={`text-sm text-[#d4d4d4]`}>{news.reporter_name}</p>
                        <p className="text-xs text-[#737373]">{news.reporter_id}</p>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                          {getNewsTypeIcon(news.news_type)}
                          <span className="text-xs text-[#737373] capitalize">{news.news_type.replace("_", " ")}</span>
                        </div>
                      </TableCell>
      ...
      <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <DialogContent className={darkMode ? "bg-[#111] border-[#1f1f1f]" : ""}>
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div>
              <label className={`text-sm font-medium ${darkMode ? "text-[#e5e5e5]" : ""}`}>
              ...
              <Input
                ...
                className={`mt-1 ${darkMode ? "bg-[#1a1a1a] border-[#262626] text-white" : ""}`}
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                {selectedReporter.photo ? (
                  ...
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center bg-[#262626]`}>
                    <User size={24} />
                  </div>
                )}
                <div>
                  <h3 className={`text-lg font-semibold ${darkMode ? "text-white" : ""}`}>{selectedReporter.name}</h3>
                  <p className="text-sm text-[#737373]">{selectedReporter.reporter_id}</p>
                  {getStatusBadge(selectedReporter.status)}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-[#737373]">Phone</p>
                ...
                <div>
                  <p className="text-[#737373]">News Submitted</p>
                  <p className={darkMode ? "text-white" : ""}>{selectedReporter.news_submitted}</p>
                </div>
              </div>
              {selectedReporter.bio && (
                <div>
                  <p className="text-[#737373] text-sm">Bio</p>
                  <p className={`text-sm ${darkMode ? "text-[#d4d4d4]" : ""}`}>{selectedReporter.bio}</p>
                </div>
              )}
            </div>
          )}
          ...
          {selectedNews && (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-[#737373] capitalize">{selectedNews.news_type.replace("_", " ")}</span>
              ...
              <div>
                <p className={`text-sm text-[#d4d4d4]`}>{selectedNews.summary}</p>
              </div>
              ...
              {selectedNews.video_url && (
                <div>
                  <p className="text-sm text-[#737373] mb-1">Video URL:</p>
                  <a href={selectedNews.video_url} target="_blank" rel="noopener noreferrer" className="text-[#7c3aed] text-sm break-all">
                    {selectedNews.video_url}
                  </a>
                </div>
              )}
              ...
              {selectedNews.reporter_video_url && (
                <div>
                  <p className="text-sm text-[#737373] mb-1">Reporter Video URL:</p>
                  <a href={selectedNews.reporter_video_url} target="_blank" rel="noopener noreferrer" className="text-[#7c3aed] text-sm break-all">
                    {selectedNews.reporter_video_url}
                  </a>
                </div>
              )}
